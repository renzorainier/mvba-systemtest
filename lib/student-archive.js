...
// Scope an enrollment query to a single student AND a single school year.
// A real LRN is unique, so it is a safe key; the shared 'TBA' placeholder is NOT,
// so for those students we rely solely on the canonical studentId (ObjectId).
// The schoolYear constraint guarantees we never touch other years' records.
const buildEnrollmentScopeFilter = ({ studentId, learnersReferenceNumber, schoolYear }) => {
  const sid = String(studentId || '');
  const lrn = normalizeLearnersReferenceNumber(learnersReferenceNumber);
  const or = [];

  if (sid) {
    or.push({ studentId: sid });
  }
  if (isResolvableLrn(lrn)) {
    or.push({ learnersReferenceNumber: lrn });
  }
  ...
  if (schoolYear) {
    filter.schoolYear = String(schoolYear);
  }
...
export async function archiveStudent(identifier, selectedSchoolYear) {
  ...
  const enrollmentScope = buildEnrollmentScopeFilter({ studentId, learnersReferenceNumber, schoolYear });
  const enrollments = await Enrollment.find(enrollmentScope).lean();
  ...
  await Enrollment.deleteMany(enrollmentScope);
...
export async function restoreStudent(identifier) {
  ...
  // Only a real, unique LRN can collide. The 'TBA' placeholder is shared by many
  // students, so it must never block a restore — those students are identified by
  ...
  const enrollmentScope = buildEnrollmentScopeFilter({
    studentId: archivedStudent.sourceStudentId || archivedStudent._id,
    learnersReferenceNumber,
    schoolYear: archivedStudent.schoolYear,
  });
  const archivedEnrollments = await ArchivedEnrollment.find(enrollmentScope).lean();
  ...
  await ArchivedEnrollment.deleteMany(enrollmentScope);
    ...
    students.map(async (student) => {
      ...
      const [enrollmentCount, paymentCount, receiptCount] = await Promise.all([
        ArchivedEnrollment.countDocuments(
          buildEnrollmentScopeFilter({
            studentId: student.sourceStudentId || student._id,
