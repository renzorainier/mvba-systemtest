...
const findArchivedStudent = async (identifier) => {
  ...
  return ArchivedStudent.findOne({ $or: query }).lean();
};
...
export async function archiveStudent(identifier) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const existingArchived = await ArchivedStudent.findById(student._id).session(session);
      if (existingArchived) {
      ...
      const archivedReceiptDocs = buildArchivedReceipts(archivedPaymentDocs);
...
export async function listArchivedStudents() {
  ...
  const students = await ArchivedStudent.find({}).sort({ archivedAt: -1 }).lean();

  const results = await Promise.all(
    students.map(async (student) => {
      const [enrollmentCount, paymentCount, receiptCount] = await Promise.all([
        ArchivedEnrollment.countDocuments({ learnersReferenceNumber: student.learnersReferenceNumber }),
