...
const archiveLeanDocs = (docs = [], schoolYear, archivedAt) => docs.map((doc) => ({ ...doc, schoolYear, archivedAt }));
...
export async function rolloverSchoolYear(currentYearId, nextYearId, promotedStudentIds = []) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const archivedStudents = archiveLeanDocs(activeStudents, normalizedCurrentYear, archivedAt);
