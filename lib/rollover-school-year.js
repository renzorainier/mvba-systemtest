...
export const runSessionWork = async (session, work) => {
  try {
...
export async function seedDraftFromActiveYear(currentYear, draftYear, session) {
  const now = new Date();
  ...
  const sourceCurriculums = await Curriculum.find({ schoolYear: currentYear }).session(session).lean();
  const legacyCurriculums = Array.isArray(settings?.curriculums)
    ? settings.curriculums.filter((curriculum) => String(curriculum.schoolYear || '').trim() === currentYear)
    : [];
  const curriculaSource = sourceCurriculums.length > 0 ? sourceCurriculums : legacyCurriculums;
  const curriculumIdMap = new Map();
  const draftCurriculums = curriculaSource.map((curriculum) => {
  ...
  const sourceGradeLevelCurriculums = await GradeLevelCurriculum.find({ school_year_id: currentYear }).session(session).lean();
  const legacyGradeLevelCurriculums = Array.isArray(settings?.gradeLevelCurriculums)
    ? settings.gradeLevelCurriculums.filter((assignment) => {
        const schoolYear = String(assignment.school_year_id || assignment.schoolYear || '').trim();
        return schoolYear === currentYear;
      })
    : [];
  const gradeLevelSource = sourceGradeLevelCurriculums.length > 0 ? sourceGradeLevelCurriculums : legacyGradeLevelCurriculums;
  const gradeLevelIdMap = new Map();
  const draftGradeLevelCurriculums = gradeLevelSource.map((assignment) => {
    const newId = new mongoose.Types.ObjectId();
  ...
  if (draftCurriculums.length > 0) await Curriculum.insertMany(draftCurriculums, { session });
  if (draftGradeLevelCurriculums.length > 0) await GradeLevelCurriculum.insertMany(draftGradeLevelCurriculums, { session });
  if (draftSections.length > 0) await Section.insertMany(draftSections, { session });
  if (draftSchedules.length > 0) await Schedule.insertMany(draftSchedules, { session });
