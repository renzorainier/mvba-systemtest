...
export async function rolloverSchoolYear(currentYearId, nextYearId, promotedStudentIds = []) {
  ...
  const activeGradeLevelCurriculums = await GradeLevelCurriculum.find({ school_year_id: normalizedCurrentYear }).lean();
  ...
  const activeCurriculums = await Curriculum.find({ schoolYear: normalizedCurrentYear }).lean();
...
export async function activateDraftSchoolYear() {
  ...
  const draftCurriculums = await Curriculum.find({ schoolYear: draftYear }).lean();
  const draftGradeLevelCurriculums = await GradeLevelCurriculum.find({ school_year_id: draftYear }).lean();
